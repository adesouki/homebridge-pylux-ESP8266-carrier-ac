...
export class PyluxCarrierAC {
  ...
  private usesFahrenheit: boolean;
  ...
  writeConfigHistory(jsonBody: string) {
    writeFileSync(this.dataFilePath, jsonBody, {
      flag: 'w',
    });
  }
  ...
  readConfigHistory() {
    ...
    this.historyFileJSON = JSON.parse(readFileSync(this.dataFilePath, 'utf-8'));
  }
  ...
  async handleActiveSet(value) {
    ...
    if (!this.historyFileJSON.lockPhysicalControls_state) {
      ...
      if (this.historyFileJSON.units == 0)
        ...
        tempValue = this.temperatureFtoC(
      ...
      const jsonBody = JSON.stringify({
        ...
        temp: tempValue, //convert and check mode first,
  ...
  async temperaturePoll(update: boolean) {
    ...
    if (res != 'ERROR') {
      ...
      if (this.historyFileJSON.units == 0)
        ...
        this.historyFileJSON.currentHeaterCooler_temp = this.temperatureCtoF(
          response.temp
        );
    }
  ...
  handleTemperatureDisplayUnitsSet(value) {
    ...
    if (
      this.historyFileJSON.currentHeaterCooler_temp < 55 &&
      this.usesFahrenheit
    ) {
      this.historyFileJSON.currentHeaterCooler_temp = this.temperatureCtoF(
        this.historyFileJSON.currentHeaterCooler_temp
      ...
      this.historyFileJSON.currentHeaterCooler_temp = this.temperatureFtoC(
        this.historyFileJSON.currentHeaterCooler_temp
      );
    }
    this.service
    ...
    if (
      this.historyFileJSON.coolingThresholdTemperature < 55 &&
      this.usesFahrenheit
    ) {
      this.historyFileJSON.coolingThresholdTemperature = this.temperatureCtoF(
        this.historyFileJSON.coolingThresholdTemperature
      );
    } else if (
      this.historyFileJSON.coolingThresholdTemperature > 55 &&
      ...
      this.historyFileJSON.coolingThresholdTemperature = this.temperatureFtoC(
        this.historyFileJSON.coolingThresholdTemperature
      );
    }
    ...
    if (
      this.historyFileJSON.heatingThresholdTemperature < 55 &&
      this.usesFahrenheit
    ) {
      this.historyFileJSON.heatingThresholdTemperature = this.temperatureCtoF(
      ...
      this.historyFileJSON.heatingThresholdTemperature = this.temperatureFtoC(
        this.historyFileJSON.heatingThresholdTemperature
      );
    }
  ...
  async handleCoolingThresholdTemperatureSet(value) {
    ...
    if (!this.historyFileJSON.lockPhysicalControls_state) {
      ...
      let tempValue = 0.0;
      if (this.historyFileJSON.units == 0) tempValue = value;
      else tempValue = this.temperatureFtoC(value);
  ...
  handleHeatingThresholdTemperatureSet(value) {
    if (!this.historyFileJSON.lockPhysicalControls_state) {
      this.historyFileJSON.coolingThresholdTemperature = value;
      let tempValue = 0.0;
      if (this.historyFileJSON.units == 0) tempValue = value; //convert
